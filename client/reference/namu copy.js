let httpCollection = document.querySelectorAll(".wiki-heading-content")[3].children[0].children[0].children[0].children
...
for (let i = 0; i < maxLength; i++) {
    ...
    namu_skeleton_table_object.name          = divideArray[i][0].children[1].children[0].children[0].innerText;
    namu_skeleton_table_object.id            = nData_id;
    namu_skeleton_table_object.cool_down     = divideArray[i][4].children[1].children[0].innerText.replace("칸", "").trim();
    namu_skeleton_table_object.unlock        = divideArray[i][5].children[1].children[0].innerText.replace(", ", "");
    namu_skeleton_table_object.place         = divideArray[i][6].children[1].children[0].innerText.replace(/\n/g, "").split(", ");
    namu_skeleton_table_object.description   = divideArray[i][7].children[1].children[0].textContent.split(".")

    for(let j = 0; j < namu_skeleton_table_object.description.length; j++) {
        if(namu_skeleton_table_object.description[j] == "") {
            delete namu_skeleton_table_object.description[j];
            continue;
        }
    
        namu_skeleton_table_object.description[j] = namu_skeleton_table_object.description[j].trim();
    }

    isaac_datas_obj[nData_id] = copyObj(namu_skeleton_table_object);
}

console.log(isaac_datas_obj);
